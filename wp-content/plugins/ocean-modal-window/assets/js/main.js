...
function oceanwpInitModal() {
	...
	$j( '.omw-open-modal, .omw-open-modal a, .omw-open-modal a.elementor-button, li.sidr-class-omw-open-modal > a' ).on( 'click', function() {
	...
	if ( typeof oceanwpLocalize.sidrSource !== 'undefined' ) {
		...
		$j( 'li.sidr-class-omw-open-modal > a' ).on( 'click', function() {
			$j.sidr( 'close', 'sidr' );
			initModal();
			return false;
		} );
